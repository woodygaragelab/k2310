...
export async function createReservation(startDate: string, endDate: string, name: string, memo: string, isCancelled: boolean, isProvisional: boolean): Promise<Reservation> {
  const res = await fetch(`${API_BASE}/reservations`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ startDate, endDate, name, memo, isCancelled, isProvisional }),
  })
  if (!res.ok) throw new Error(`Failed to create reservation: ${res.status}`)
  return res.json()
}

export async function updateReservation(id: string, startDate: string, endDate: string, name: string, memo: string, isCancelled: boolean, isProvisional: boolean): Promise<Reservation> {
